...
| PAdES-T/LT/LTA | 🔴 | 🟡 (T y LT; LTA no) | PAdES-T real (RUNBOOK 12.14); PAdES-LT real desde RUNBOOK 12.24 — DSS/VRI embebido, probado con 5 pruebas unitarias (incluye multifirma), no verificado en vivo con DNIe real por falta de escenario Docker+PKCS11. LTA (archive timestamp) no iniciado |
...
7. ~~PAdES-LT (DSS/VRI embebido)~~ — hecho (RUNBOOK 12.24), probado con 5 pruebas unitarias (incluye documentos multifirma). No verificado en vivo con una firma DNIe/PKCS11 real (necesita el escenario nativo fuera de Docker de la sección 5). Queda PAdES-LTA (archive timestamp) y una prueba de integración a nivel `FirmarLocalHandler`, ambos deliberadamente pospuestos (ver RUNBOOK 12.24, "alcance no cubierto").
