...
| Secretos productivos | 🔴 | 🟡 | La llave de firma JWT ya está externalizada (`RsaKeyStore`, fuera de `appsettings.json`, RUNBOOK 12.21). `client_secret` de integradores demo ahora se guarda como hash Argon2id, nunca en texto plano (RUNBOOK 12.23). `Jwt:SecretoClienteInterno` ahora tiene rotación sin corte (lista de secretos aceptados en el Gateway) y el servicio se niega a arrancar fuera de Development con un secreto `dev-only-` o menor a 32 caracteres (RUNBOOK 12.27); el valor productivo lo entrega el operador por variable de entorno. Sigue pendiente: un secreto por servicio (hoy uno compartido), almacén de secretos integrado y endpoint de gestión de `client_secret` |
