...
**Limitación deliberada — más amplia de lo que parece a primera vista**: `PdfSignaturePlaceholder` hace una reescritura completa del PDF (no una actualización incremental en el sentido estricto de ISO 32000, revisión N sobre N-1) cada vez que se llama. Para un documento que **nunca tuvo ninguna firma PAdES antes** (el caso normal), esto es perfectamente válido — el `/ByteRange` describe correctamente el archivo final. Pero la prueba con DNIe real de esta sesión reveló el caso general: el PDF de prueba usado (`0 ENBLANCO.pdf`) **ya traía una firma digital previa incrustada de antes de subirlo a SecureSign** (no era un PDF en blanco sin firmar, pese al nombre) — la reescritura invalidó esa firma anterior (offsets corridos, `message-digest attribute value does not match calculated value` al verificarla), mientras que la firma NUEVA (la del DNIe, agregada al final del archivo) verificó correctamente. Es decir: la limitación no es solo "un segundo firmante de SecureSign" — es **cualquier firma PAdES que el documento ya trajera al momento de subirlo**, sin importar su origen. Mitigación real (no implementada aún): detectar si el documento ya tiene un `/Sig` con `/ByteRange` antes de reescribir y, si lo tiene, hacer una actualización incremental de verdad en vez de una reescritura completa.
