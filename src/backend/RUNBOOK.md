...
**Verificado**: 11 pruebas nuevas (`tests/SecureSign.UnitTests/Auth/SecretoInternoTests.cs`; 128 en total, 0 regresiones, build sin advertencias): aceptación del secreto actual, actual y nuevo durante la rotación, rechazo del retirado, rechazo sin encabezado, un secreto vacío en la lista no autoriza un encabezado vacío, y la regla de arranque (ejemplo permitido en Development, rechazado fuera; secreto corto; secretos adicionales también revisados; secretos largos reales aceptados; servicio sin secreto no falla). Ejecutando el Gateway real con `dotnet run`: en `Production` con el valor de ejemplo termina con `OptionsValidationException` al arrancar; en `Production` con `Jwt__SecretoClienteInterno` de 46 caracteres arranca y escucha; en `Development` arranca con el valor de ejemplo. **No se verificó contra el stack Docker**: Docker Desktop se cayó durante el build (mismo problema de RUNBOOK 12.16) — el cambio de `docker-compose.yml` está sin ejercitar en contenedores.
