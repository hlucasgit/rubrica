...
### 12.21 Migración JWT: de llave HS256 compartida a RS256 con el Gateway como único firmante (informe, hallazgo P1 "Autenticación productiva")

Los 7 servicios (Gateway + 6 APIs de dominio) compartían **la misma llave simétrica HS256**, literalmente `dev-only-signing-key-do-not-use-in-production-change-me-32bytes`, copiada en 7 `appsettings.json` — cualquiera de los 7 podía forjar un token válido para cualquier otro. El informe pide IdP/STS real, firma asimétrica, rotación de llaves vía JWKS y secretos externalizados. Traer un IdP externo completo (Keycloak/Duende) queda fuera de alcance de una sesión — se construyó el STS mínimo real que sí cabe: **el Gateway es el único componente que conserva la llave privada; todo lo demás valida contra una llave pública publicada, o le pide al Gateway que firme algo por él.**

**`RsaKeyStore`** (nuevo, `SecureSign.Shared.Auth`) — llaves RSA-2048 persistidas como archivos PEM en un directorio (`Jwt:DirectorioLlaves`), deliberadamente fuera del repositorio y de `appsettings.json` — esto es lo que cierra "secretos externalizados" para el material de firma en sí. Si el directorio está vacío, genera una llave sola al arrancar. El nombre de archivo (`AAAAMMDDHHmmss_kid.pem`) ordena cronológicamente por string, sin depender de metadatos de filesystem (poco fiables de forma uniforme entre Windows/Linux/contenedores) — la llave activa es siempre la de nombre más alto. Publica TODAS las llaves vigentes, no solo la activa: eso es lo que hace posible rotar (agregar una llave nueva, borrar la vieja tras un período de gracia documentado) sin reconfigurar ningún servicio.

**Gateway como STS**:
- `GET /.well-known/openid-configuration` y `GET /.well-known/jwks.json` (`OidcController`, nuevo) — publican issuer, jwks_uri y las llaves públicas vigentes.
- `JwtTokenService` pasa de `HmacSha256` a `RS256` con la llave activa de `RsaKeyStore`.
- Nuevo endpoint `POST /api/auth/interno/emitir` — firma cualquier conjunto de claims que le pase un servicio interno autenticado con un secreto compartido (`Jwt:SecretoClienteInterno`, header `X-Internal-Client-Secret`, nunca un Bearer). Es el ÚNICO lugar del sistema que firma algo que no sea el token externo — lo usan tanto el intercambio de tokens interno como los tickets del Firmador Local, sin duplicar lógica de firma en cada servicio.

**`AddSecureSignJwtValidation`** — dejó de construir `SymmetricSecurityKey` desde una llave compartida. **Primer intento, fallido**: usar `JwtBearerOptions.Authority` + descubrimiento OIDC automático de ASP.NET Core — en la verificación en vivo, el propio Gateway validando SU PROPIO token (autoridad = él mismo, `http://gateway:8080`) fallaba con `IDX10500: Signature validation failed. No security keys were provided`, sin ningún diagnóstico aprovechable pese a que el JWKS era perfectamente alcanzable (confirmado con un contenedor `curlimages/curl` en la misma red). **Corrección**: se abandonó el `ConfigurationManager` automático de Microsoft y se escribió `JwksLlaveResolver` (propio, cachea 5 minutos) conectado vía `TokenValidationParameters.IssuerSigningKeyResolver` — mismo resultado funcional, con cada paso logueable. Con esto sí validó correctamente, primera vez.

**`TokenExchangeService`/`EmisorTicketFirmaLocal`** — pasaron de auto-firmar localmente a llamar `POST /api/auth/interno/emitir` (vía `EmisorTokenInterno`, nuevo). Firma pública sin cambios (`Exchange`/`EmitirTokenDeSistema` siguen tomando los mismos parámetros, solo ahora son `async`) — `TokenExchangeHandler` y cada `AddSecureSignInternalHttpClient` no necesitaron enterarse de cómo se firma un token, solo que lo reciben.

**Segundo bug real, encontrado en la MISMA verificación en vivo**: con el resolver propio funcionando, la cadena Firma → Identidad seguía devolviendo 401 — pero esta vez SIN ningún log de rechazo en el servicio destino, indicando que el header `Authorization` nunca llegaba. Diagnosticado emitiendo un token manualmente y probándolo directo contra Identity.Api (`docker run --rm --network backend_default curlimages/curl ...`): validaba perfecto. Aislado a `EmisorTokenInterno`: el Gateway responde en snake_case (`access_token`/`expires_in`, misma convención que `POST /api/auth/token`), pero el DTO privado de respuesta usaba propiedades `AccessToken`/`ExpiresIn` sin mapeo — `System.Text.Json`, incluso con `JsonSerializerDefaults.Web`, NO empareja `access_token` con `AccessToken` (el naming policy de camelCase solo cambia mayúsculas/minúsculas, nunca inserta o quita guiones bajos), así que `AccessToken` quedaba `null` en silencio. `TokenExchangeHandler` construía igual un header `Authorization: Bearer` (vacío), que el destino trataba como "sin token", sin excepción ni log en ningún lado. **Corregido** con `[JsonPropertyName("access_token")]`/`[JsonPropertyName("expires_in")]` explícitos, más una comprobación fail-closed nueva en `EmisorTokenInterno.EmitirAsync`: si `AccessToken` viene vacío, lanza en vez de dejar que se construya un Bearer vacío — para que esta clase de bug nunca vuelva a fallar en silencio.

**Verificado en vivo, de punta a punta, contra el stack Docker real** (build secuencial de los 7 servicios por la inestabilidad conocida de Docker Desktop — se cayó 3 veces durante esta sección, siempre recuperado con el workaround de 12.16):
1. `GET /.well-known/jwks.json` — llave RSA real publicada, generada sola al primer arranque.
2. `POST /api/auth/token` — header del JWT confirma `"alg":"RS256"` y el mismo `kid` publicado en el JWKS.
3. Una ruta protegida (`requerir-token`, validada por el propio Gateway contra sí mismo) — `401` con el `ConfigurationManager` automático, `200 OK` con `JwksLlaveResolver`.
4. Flujo real de firma de punta a punta (subir documento → crear solicitud → visualizar → firmar) — exitoso, ejercitando Firma→Identidad, Firma→Documentos, Firma→Criptografía (proveedor nativo en el host) y Firma→Evidencia, TODOS vía intercambio de tokens real contra el Gateway.
5. Ticket del Firmador Local emitido vía el mismo endpoint interno, y aceptado como Bearer válido por el pipeline de validación normal (`GET /api/firmas/{id}/estado` con el ticket como credencial → `200 OK`).

**Pruebas nuevas** (`tests/SecureSign.UnitTests/Auth/`, 13 pruebas, 79 en total): `RsaKeyStoreTests` (genera sola, recarga sin regenerar, la activa es la más reciente por nombre no por filesystem, `KeyId` correcto), `JwtRoundTripTests` (firma con una llave, valida con su pública — funciona; valida con la pública de OTRO llavero — falla, con el bug de reutilizar el mismo directorio de prueba encontrado y corregido en el camino), `EmisorTokenInternoTests` (prueba EXACTA del bug real de mapeo snake_case, con la respuesta real del Gateway, no una aproximación — más el caso 200-sin-token-utilizable que ahora lanza en vez de producir un Bearer vacío), `OidcControllerTests` (forma real de discovery/JWKS, y que la llave privada nunca aparece publicada).

**Alcance no cubierto, documentado explícitamente**:
- IdP externo real (Keycloak/Duende) — el Gateway sigue siendo un STS mínimo propio.
- Authorization Code + PKCE para usuarios humanos — la plataforma solo usa `client_credentials` (B2B) hoy.
- Secreto de cliente interno POR SERVICIO — hoy sigue siendo uno compartido (`Jwt:SecretoClienteInterno`), más angosto que la llave de firma de antes (solo sirve para pedir, nunca para firmar) pero no perfecto.
- Disparador automático de rotación de llaves por tiempo — el mecanismo (multi-llave + JWKS) queda listo; rotar en sí es un procedimiento manual: dejar caer un archivo `.pem` nuevo en el volumen `securesign_gateway_llaves`, y borrar el viejo pasado un período de gracia.
- Hash de `client_secret` de integradores demo (Argon2id) — concern relacionado pero separado, no tocado aquí.

### 12.22 Verificación de la firma XAdES de la TSL de INDECOPI — y un hallazgo real sobre el propio dato oficial (informe, hallazgo P1 "Lista de confianza IOFE")

`ListaConfianzaIofe` (`SecureSign.Trust`) carga la Trust Service Status List oficial (ETSI TS 119 612, `https://iofe.indecopi.gob.pe/TSL/tsl-pe.xml`) para responder "¿esta cadena de certificación pertenece a la IOFE?". Hasta ahora se cargaba el XML tal cual, sin verificar que la propia TSL viniera firmada por INDECOPI — cualquiera con acceso a alterar el archivo en tránsito (HTTP en vez de HTTPS, un proxy) o en reposo (el archivo local) podía inyectar o quitar una Entidad de Certificación acreditada, y el sistema lo hubiera aceptado igual.

**`VerificadorFirmaTsl`** (nuevo, `internal` a `SecureSign.Trust`) — verifica la firma XAdES-BES (XML-DSig por debajo, envolvente) de la TSL contra un ancla de confianza real, en dos pasos, en este orden:
1. Extrae el certificado firmante de `<ds:KeyInfo>/<ds:X509Certificate>` y construye su cadena contra el ancla con `X509Chain`/`CustomRootTrust` (mismo patrón que `ValidadorCertificados` para certificados de firmante). Si el certificado firmante no fue emitido por el ancla esperada, rechaza **sin siquiera intentar la verificación criptográfica** — confiar en la firma de un certificado no emitido por la autoridad esperada sería tan inseguro como no verificar nada.
2. Solo si la cadena es válida, llama `SignedXml.CheckSignature(certificado, verifySignatureOnly: true)` para confirmar que ningún byte del documento cambió desde que se firmó.

`CryptoConfig.AddAlgorithm` (necesario para que `SignedXml` reconozca las URIs `rsa-sha256` y C14N 1.0 — el paquete standalone `System.Security.Cryptography.Xml` 8.0.4 no las trae pre-registradas, a diferencia del framework compartido de .NET Framework clásico) exige un tipo **público** — el equivalente interno de Microsoft (`RSAPKCS1SHA256SignatureDescription`) no es accesible desde fuera de su ensamblado, así que se reimplementó una versión propia mínima, con los mismos valores confirmados por reflexión contra el tipo real antes de escribir el código (no adivinados) — en particular `KeyAlgorithm` debe ser `AssemblyQualifiedName`, no `FullName`: usar `FullName` produce un `NullReferenceException` dentro de `SignedXml.IsKeyTheCorrectAlgorithm`, un error que no señala en absoluto la causa real.

**El ancla de confianza** (`tsl-firmante-raiz.crt`, nuevo, junto a `tsl-pe.xml`) es la raíz real de INDECOPI ("INDECOPI AAC RAIZ"), obtenida de la URL AIA oficial embebida en el propio certificado firmante de la TSL (`https://recursos.indecopi.gob.pe/iofe/crt/ca_root_indecopi.crt`) — deliberadamente **fuera** de `ConfianzaIofe/raices/`, que alimenta `AlmacenRaicesConfiables` (las raíces que emiten DNIe, para validar certificados de firmante). Son jerarquías PKI completamente distintas — quién puede firmar la TSL no tiene nada que ver con quién puede emitir un DNIe — y mezclarlas ampliaría sin querer el conjunto de raíces confiables para validar firmantes.

**Hallazgo real, no un bug del código**: verificado contra el archivo real de producción, `VerificarOLanzar` lanza — la TSL que INDECOPI publica hoy **no verifica contra su propio certificado embebido**. Antes de aceptar esta conclusión se descartaron, en orden, las tres explicaciones más probables de que fuera un bug propio:
1. **Corrupción local o descarga defectuosa** — descartado: una descarga fresca (`curl` directo a la URL oficial) es byte-idéntica al archivo del repositorio.
2. **Bug en el cálculo del digest** — descartado: se recalculó el hash de los tres `<ds:Reference>` del documento (certificado firmante, propiedades XAdES firmadas, documento completo) por reflexión contra el método interno real de .NET (`Reference.CalculateHashValue`), y los tres coinciden con lo esperado.
3. **El motor XML-DSig de .NET está roto** — descartado: un documento análogo, autofirmado y autoverificado con `SignedXml` en el mismo proceso, verifica correctamente (son las pruebas 1-3 de `VerificadorFirmaTslTests`, que fuerzan exactamente este mecanismo con fixtures propios).

Con las tres descartadas, la conclusión es que el hallazgo está en el pipeline de publicación de INDECOPI, no en este código. **Por eso `ListaConfianzaIofe.CargarDesdeArchivoFirmado` no está conectado a `Program.cs` de `Signature.Api`** — que sigue usando `CargarDesdeArchivo` (sin verificar) — activar la verificación como bloqueante hoy tumbaría el arranque del servicio contra un dato público real, no contra un archivo corrupto localmente. `CargarDesdeArchivoFirmado` queda lista, probada, y documentada para activarse en cuanto INDECOPI corrija su pipeline (o para reportarse a INDECOPI como hallazgo, según decida el equipo de cumplimiento).

**Bug propio real encontrado y corregido en el camino** (distinto del hallazgo de arriba): el fixture de prueba que firma un XML sintético con `SignedXml` fallaba en el caso de "firma correcta, debería verificar" — no por un error en `VerificadorFirmaTsl`, sino porque `XmlDocument.Save()` indenta el XML por defecto cuando `PreserveWhitespace` es `false` (el estado del documento al momento de firmar); al recargar ese archivo con `PreserveWhitespace = true` (necesario para igualar cómo se procesa la TSL real), esa indentación se vuelve texto significativo y rompe la canonicalización C14N, dando un falso "no verifica". Aislado comparando tres variantes (verificar en memoria sin recargar / recargar sin preservar espacios / recargar preservando espacios) en un proyecto de diagnóstico descartable — se corrigió fijando `PreserveWhitespace = true` en el documento ANTES de guardarlo, para que `Save()` no inserte formato.

**Pruebas nuevas** (`tests/SecureSign.UnitTests/Trust/VerificadorFirmaTslTests.cs`, 4 pruebas, 83 en total): TSL sin firma lanza; certificado firmante no emitido por el ancla lanza sin intentar la criptografía (con un ancla no relacionada, para probar que el rechazo es por cadena, no por firma); TSL firmada correctamente por un certificado emitido por el ancla verifica y carga; y una prueba de regresión contra el archivo real de INDECOPI que documenta el hallazgo — si INDECOPI corrige su pipeline algún día, esta prueba empieza a fallar, señal de que ya se puede activar `CargarDesdeArchivoFirmado` en producción.

**Alcance no cubierto, documentado explícitamente**:
- Validación XAdES-BES estricta en el sentido de ETSI TS 101 903 — no se cruza el digest de `SigningCertificate` dentro de `SignedProperties` contra el certificado real de `KeyInfo` (comprobación XAdES adicional, no XML-DSig puro).
- Verificación de marcas de tiempo o revocación del propio certificado firmante de la TSL.
- Activación en producción — bloqueada por el hallazgo de arriba, no por falta de código.

### 12.23 Hash Argon2id del `client_secret` de integradores demo — y un bug real de off-by-one encontrado por la propia prueba de round-trip

`docs/07-seguridad/modelo-seguridad.md` sección 6 pide explícitamente que el `ClientSecret` de aplicaciones integradoras se guarde "solo como hash (Argon2id)". Hasta ahora `ClientesDemo:Clientes[].ClientSecret` vivía en texto plano en `appsettings.json` (`"demo-secret-not-for-production"`) — cualquiera con acceso a ese archivo (o a la variable de entorno que lo sobrescribe en un despliegue real) obtenía directamente una credencial utilizable contra `POST /api/auth/token`, sin ningún paso intermedio.

**`Argon2idSecretHasher`** (nuevo, `SecureSign.Shared.Auth`) — `Hashear(secreto)`/`Verificar(secreto, hash)` sobre BouncyCastle (`Argon2BytesGenerator`/`Argon2Parameters`, ya usado en `SecureSign.Trust` — se reutiliza la misma librería del repo en vez de sumar una dependencia nueva solo para esto). Parámetros m=19 MiB/t=2/p=1 (mínimo recomendado por OWASP para Argon2id en un proceso de servidor sin hardware dedicado) guardados JUNTO con el hash, en un formato estilo PHC (`$argon2id$v=19$m=19456,t=2,p=1$<salt base64>$<hash base64>`) — así se puede subir el costo en el futuro sin invalidar los hashes ya emitidos, porque `Verificar` siempre relee los parámetros del propio hash, nunca usa las constantes actuales a ciegas.

**`ClienteDemo.ClientSecret` (string) → `ClienteDemo.SecretosHash` (`List<string>`)** — una lista, no un solo valor: de regalo permite rotar sin downtime (agregar el hash del secreto nuevo, mantener el viejo activo durante la ventana de gracia, después quitarlo) sin ningún cambio de código ni de esquema adicional. `AuthController.ObtenerToken` pasó de comparar el secreto en tiempo constante contra un único valor a `cliente.SecretosHash.Any(hash => Argon2idSecretHasher.Verificar(client_secret, hash))` — la comparación en tiempo constante sigue existiendo, ahora dentro de `Verificar` mismo.

**Bug real, encontrado por la propia batería de pruebas antes de tocar producción**: la primera versión de `Argon2idSecretHasher.Verificar` devolvía `false` SIEMPRE, incluso con el secreto correcto — un off-by-one en el parseo de los parámetros embebidos (`"m=19456"`: se leía `AsSpan(1)` → `"=19456"`, con el signo `=` incluido, en vez de `AsSpan(2)` → `"19456"`; `int.TryParse` sobre `"=19456"` falla, así que `TryParsear` rechazaba CUALQUIER hash bien formado y `Verificar` caía siempre por la rama "formato inválido → no coincide"). Encontrado inmediatamente por la primera prueba de round-trip (`Hashear` seguido de `Verificar` con el mismo secreto, que debía dar `true` y daba `false`) — antes de que este código llegara a generar el hash real para `appsettings.json`, y muchísimo antes de intentarlo contra el Gateway real. Corregido validando además que exista el signo `=` en la posición esperada, no solo el prefijo (`m`/`t`/`p`).

**Verificado en vivo contra el stack Docker real** (build secuencial de los 7 servicios, Docker Desktop se cayó 3 veces durante esta sección — mismo patrón de siempre, recuperado cada vez reiniciándolo y reintentando el build secuencial, workaround de 12.16):
1. `POST /api/auth/token` con `client_id=sgd-demo` y el secreto real en texto plano (`demo-secret-not-for-production`) contra el hash Argon2id ahora almacenado en `appsettings.json` — `200 OK`, token RS256 válido emitido.
2. Mismo request con un secreto incorrecto — `401 Unauthorized`.
3. El token del paso 1 contra una ruta protegida del Gateway (`GET /api/documentos`) — pasa autenticación (`405 Method Not Allowed`, no `401`: el rechazo es por método HTTP, no por el token).

**Pruebas nuevas** (`tests/SecureSign.UnitTests/Auth/`, 15 pruebas, 98 en total): `Argon2idSecretHasherTests` (round-trip correcto, secreto incorrecto falla, el hash nunca contiene el secreto en texto plano, dos hashes del mismo secreto son distintos por la sal aleatoria, una batería de formatos de hash inválidos que deben fallar sin lanzar — incluida exactamente la forma que dispara el bug de arriba si reapareciera —, y una prueba fija contra el hash real que quedó en `appsettings.json` para detectar si algún día se regenera con un algoritmo incompatible); `AuthControllerTests` (el flujo `client_credentials` completo con `Argon2idSecretHasher` real, no un doble de prueba — para que un bug como el de arriba se detecte también a nivel de controlador, no solo en el hasher aislado).

**Alcance no cubierto, documentado explícitamente**:
- Rotación real de secretos (dos hashes activos durante una ventana de gracia) — el mecanismo (`SecretosHash` como lista) queda listo, pero no hay ningún procedimiento ni endpoint para gestionarla todavía; es manual, igual que la rotación de llaves JWT (RUNBOOK 12.21).
- `Jwt:SecretoClienteInterno` sigue en texto plano en `appsettings.json` — es un secreto de arquitectura interna (autentica la llamada a `POST /api/auth/interno/emitir`), no un `client_secret` de integrador, y queda fuera del alcance de este hallazgo específico del informe.

### 12.24 PAdES-LT: Document Security Store (DSS/VRI) embebido — validación a largo plazo sin depender de que la CRL original siga publicada (informe, "PAdES-T/LT/LTA")

Hasta 12.14 el sistema llegaba a PAdES-T (sello de tiempo RFC 3161) pero no a LT: la firma se podía verificar matemáticamente y su instante de sellado era confiable, pero **validar la CADENA de confianza (vigencia, cadena X.509, CRL/OCSP) solo era posible mientras esos datos siguieran disponibles en línea** — una CRL que RENIEC rota cada 24h, o un certificado que ya venció, dejaban sin forma de reconstruir "¿esto era de fiar en el momento de firmar?" años después, que es exactamente lo que un validador PAdES-LT debe poder responder sin red.

**Diseño**: `PdfDssWriter` (nuevo, `SecureSign.Pades`) agrega, como UNA revisión incremental más (mismo mecanismo ISO 32000-1 §7.5.6 que `PdfSignaturePlaceholder`, RUNBOOK 12.8), un diccionario `/DSS` en el Catálogo con la cadena de certificados y la CRL/OCSP que **ya se habían obtenido y verificado** durante la validación de confianza (RUNBOOK 12.9/12.17) — nunca se descarga ni se vuelve a pedir nada aparte. Estructura confirmada contra fuentes primarias antes de escribir una sola línea (no adivinada — mismo rigor que 12.22): el Adobe Supplement to ISO 32000 (adoptado por ISO 32000-2 §12.8.4.3) y el código real de `LtvVerification.java` de iText (implementación de referencia ampliamente usada) — `/Certs`/`/CRLs`/`/OCSPs` como arrays de referencias a objetos stream con el DER crudo, y `/VRI` como un diccionario por firma cuya CLAVE es el hexadecimal en MAYÚSCULAS del hash **SHA-1 del valor BINARIO exacto** de `/Contents` de esa firma (nunca del texto hex que aparece en el PDF).

**`SecureSign.Trust` ahora expone, no solo descarta, el material crudo**: `VerificadorRevocacionCrl`/`VerificadorRevocacionOcsp` devuelven además los bytes DER de la CRL/respuesta OCSP realmente usada (antes solo devolvían el veredicto); `ValidadorCertificados` los junta con la cadena de certificados ya construida (`X509Chain.ChainElements`) en un nuevo `MaterialValidacionLargoPlazo`. La cadena Application→Infrastructure de `SecureSign.Signature` mapea este tipo a un equivalente propio de Application (`ResultadoConfianzaFirmante.MaterialLargoPlazo`) en vez de referenciar `SecureSign.Trust` directamente desde esa capa — mismo respeto por Clean Architecture que ya regía ahí.

**`FirmarLocalHandler`** llama `PdfDssWriter.AgregarDss` justo después de confirmar que el PAdES incrustado es válido, con el `MaterialLargoPlazo` que la MISMA llamada a `validadorConfianza.ValidarAsync` de más arriba ya había obtenido — **best-effort, igual que el sello de tiempo (RUNBOOK 12.14)**: si algo falla acá, el documento sigue siendo un PAdES-B/T perfectamente válido, nunca se aborta la firma real por un fallo de esta mejora.

**Multifirma (hasta 20 firmantes, RUNBOOK 12.11) queda cubierto**: `PdfDssWriter` detecta si el PDF YA tiene un `/DSS` de un firmante anterior (mismo principio que el lector propio de `PdfSignaturePlaceholder`: solo entiende un `/DSS` que este mismo componente escribió antes, nunca uno de origen arbitrario) y lo EXTIENDE — conserva las entradas `/VRI` y las referencias `/Certs`/`/CRLs`/`/OCSPs` de revisiones anteriores (esos objetos nunca se tocan ni se borran, incremental de verdad) y agrega las de la firma nueva. Puede repetir certificados compartidos entre revisiones sucesivas (p. ej. la misma raíz) — el formato lo permite y evita tener que decodificar el contenido de streams ya escritos solo para deduplicarlos.

**Detalle no trivial resuelto con bracket-matching, no con "buscar el último `>>`"**: tanto para insertar `/DSS N 0 R` en un Catálogo que puede traer otros diccionarios anidados (`/Names`, `/MarkInfo`, etc., según el PDF de origen) como para extraer el contenido interno de un `/VRI` viejo al fusionar, una búsqueda ingenua del último `>>` se rompe apenas hay anidamiento. `EncontrarCierreDiccionario` cuenta profundidad (`<<`/`>>` como tokens) para encontrar el cierre que de verdad corresponde a la apertura dada — correcto sea cual sea la estructura del diccionario, no solo la que este código genera.

**Pruebas nuevas** (`tests/SecureSign.UnitTests/Pades/PdfDssWriterTests.cs`, 5 pruebas, 103 en total, verificación deliberadamente INDEPENDIENTE — parseo propio del PDF resultante en el test, nunca reutilizando los parsers de producción, para no esconder un bug compartido): cadena de certificados + CRL embebidos con los bytes exactos y la clave VRI correcta (recalculada de forma independiente); agregar DSS no invalida la firma ya existente (regresión real — un DSS mal construido podría, en teoría, alterar bytes cubiertos por el `/ByteRange` si el código tuviera un bug de límites); sin CRL ni OCSP los arrays quedan vacíos, no con entradas falsas; **dos firmantes, cada DSS extiende al anterior sin perder ninguna entrada** (la prueba de la fusión, con las CLAVES VRI de ambos firmantes verificadas explícitamente presentes al final, y las DOS firmas del documento siguiendo válidas); PDF sin ninguna firma previa lanza (no hay `/Contents` del que calcular una clave VRI). Un bug real de los propios TESTS (no de producción) se encontró y corrigió en el camino: el parser de verificación asumía un `\n` antes de cada objeto nuevo, pero el primer objeto de una revisión queda pegado directamente al `%%EOF` de la revisión anterior sin separador — así lo escribe también `PdfSignaturePlaceholder` desde antes de esta sesión (funciona porque los lectores PDF reales saltan por offset de xref, nunca escanean secuencialmente); corregido con un patrón que no exige el `\n` inicial.

**Limitación real del entorno, no del código**: RENIEC exige acceso regulado por su TUPA para consultar OCSP (ver RUNBOOK 12.9/comentario de clase de `VerificadorRevocacionOcsp`) — mientras SecureSign no tenga ese acceso, `OcspRespuestaDer` será `null` casi siempre y el DSS solo llevará prueba CRL, nunca OCSP. Esto es exactamente lo esperado: el propio diseño de `EstadoFinal` en `ResultadoValidacionCertificado` ya exige que AL MENOS uno de los dos (CRL u OCSP) haya confirmado "no revocado" — con RENIEC, hoy, ese uno siempre es la CRL.

**Alcance no cubierto, documentado explícitamente**:
- El diccionario `/Extensions` (ESIC) que declara el nivel de extensión ISO 32000-2 en el Catálogo — es metadata de descubrimiento para herramientas que la consultan, no la parte que hace posible la validación offline en sí (el `/DSS`/`/VRI` reales). Omitido deliberadamente, no por descuido.
- PAdES-LTA (archive timestamp sobre el propio DSS, para sobrevivir a que el algoritmo de firma o el certificado de la TSA queden débiles/vencidos con el tiempo) — LT resuelve la validación a largo plazo del ESTADO de revocación; LTA es un endurecimiento adicional sobre el tiempo, no iniciado.
- Prueba de integración a nivel `FirmarLocalHandler` (con dobles de todas sus dependencias) que confirme el wireo específico del `try/catch` nuevo — se evaluó y se decidió no escribirla esta ronda: el mecanismo real (`PdfDssWriter`) ya está probado a fondo de forma aislada, y armar el harness completo de `FirmarLocalHandler` (repositorio, identidad, documentos, evidencias, auditoría, la máquina de estados de `SolicitudFirma`) es una inversión de prueba grande para cubrir lo que en los hechos es un `try/catch` de tres líneas.
- Verificación en vivo contra el stack Docker real con una firma DNIe/PKCS11 genuina — no fue posible en esta sesión (necesita el escenario nativo fuera de Docker ya documentado en RUNBOOK 12.9, más Docker Desktop inestable durante esta fase). La cobertura real de esta fase es la batería de pruebas unitarias de arriba, no una corrida en vivo end-to-end.

### 12.25 PAdES-LTA: sello de tiempo de ARCHIVO sobre la firma + el DSS (informe, "PAdES-T/LT/LTA", cierre completo)

Con 12.24 el sistema llegaba a PAdES-LT: la firma se podía validar sin depender de que la CRL original siguiera publicada. Pero el propio DSS embebido no tenía ninguna prueba independiente de CUÁNDO se agregó — si el algoritmo RSA/SHA-256 de la firma, o el certificado de la TSA de PAdES-T, quedaran débiles o vencidos con los años, no habría forma de demostrar que el documento (con su DSS) ya existía ANTES de que eso pasara. PAdES-LTA cierra exactamente eso: un sello de tiempo de ARCHIVO, sobre TODO el documento tal como quedó después de firmar y embeber el DSS.

**Estructura confirmada contra fuente primaria antes de escribir código** (misma disciplina que 12.22/12.24): un `/DocTimeStamp` es, en el PDF, el mismo mecanismo `/ByteRange`+`/Contents`+widget/AcroForm que un `/Sig` normal, pero con `/Type /DocTimeStamp` y `/SubFilter /ETSI.RFC3161` — y su `/Contents` es directamente el `TimeStampToken` RFC 3161 (nunca un CMS de firma con `SignerInfo` propio sobre el contenido). Como un sello de archivo solo tiene sentido sobre un PDF que YA tiene al menos una firma, `PdfSignaturePlaceholder.PrepararSelloDeArchivo` (nuevo) reutiliza exclusivamente la ruta del lector propio — nunca PdfSharpCore — factorizada de `PrepararConLectorPropio` mediante un delegado (`Func<int, string>`) para el objeto principal, sin duplicar la mecánica de xref/widget/AcroForm que ya estaba probada.

**Verificación distingue un `/DocTimeStamp` de una firma real, no lo trata como una CMS más**: `PdfSignatureVerifier` detecta `/SubFilter /ETSI.RFC3161` (por texto, igual que ya detecta `/Name`/`/M`) ANTES de intentar `CmsSignedData` — de lo contrario daría siempre "message-digest inválido" (el mensaje que la TSA realmente firmó es el `TSTInfo`, no el contenido cubierto directamente). `VerificarSelloDeArchivo` (nuevo) exige DOS cosas para `Valido=true`: que la firma interna del token sea consistente (`TimeStampToken.Validate`, igual que `SecureSign.Tsa.VerificadorTokenTsa`) Y que su `messageImprint` coincida con el SHA-256 del contenido REALMENTE cubierto — un `/DocTimeStamp` cuyo token es internamente válido pero fue tomado de OTRO documento (o el documento cambió después) se rechaza igual. `ResultadoVerificacionPades` gana `EsSelloDeArchivo` para que ningún consumidor confunda el certificado de la TSA con el de un firmante.

**`SecureSign.Validator` separa los sellos de archivo de las firmas reales, no los mezcla**: `ValidadorDocumentoPades.ValidarAsync` filtra por `EsSelloDeArchivo` ANTES de evaluar confianza IOFE — evaluar el certificado de la TSA contra la TSL de IOFE sería una categoría equivocada (la TSA no es un firmante acreditado). Nuevo `ResultadoValidacionSelloArchivo`/`SelloArchivoDto` (Application) para reportarlos aparte, sin afectar `DocumentoValido` (igual que el sello de PAdES-T, informativo por ahora).

**`FirmarLocalHandler`** pide el sello de archivo justo después de embeber el DSS, con `SecureSign.Tsa.ClienteTsaRfc3161` (el MISMO cliente puro RFC 3161 de PAdES-T, RUNBOOK 12.14 — ahora también usado del lado servidor, no solo desde el Firmador Local) — mismo best-effort de siempre: un fallo acá nunca invalida la firma real ya confirmada.

**Deuda real encontrada y cerrada de paso, no ignorada**: al agregar `SecureSign.Tsa.OpcionesTsa` (compartido) apareció una colisión de nombres con `SecureSign.Crypto.Infrastructure.OpcionesTsa` — una copia CASI IDÉNTICA que ya existía, registrada en DI de `SecureSign.Crypto.Api` pero sin ningún consumidor real (`ISellosTiempoProvider` nunca se inyecta en ningún handler — scaffolding de un diseño anterior, superado por el cliente directo que sí usa RUNBOOK 12.14). En vez de esquivar la colisión con un alias, se eliminó la copia dormida de Crypto y se migró `ProveedorSellosTiempoRfc3161` a la clase compartida — una duplicación real menos en el repositorio, no una más.

**Verificado**: 6 pruebas nuevas (`tests/SecureSign.UnitTests/Pades/PdfSelloArchivoTests.cs`, con la misma "TSA de prueba offline" 100% determinística de `SelloTiempoTests`/RUNBOOK 12.14, sin red) — sello válido se detecta como tal (no como firma) y verifica; agregar el sello no invalida la firma real anterior; el sello también cubre un DSS previo (RUNBOOK 12.24); el documento alterado DESPUÉS de sellarse invalida el sello (mensaje de error explícito de mismatch de `messageImprint`); un token alterado no verifica; sin ninguna firma previa, `PrepararSelloDeArchivo` lanza. 109 pruebas en total, 0 regresiones. Verificado en vivo contra el stack Docker real (build secuencial de los 7 servicios, sin caídas esta vez): `Signature.Api` y `Crypto.Api` arrancan limpios con el `ClienteTsaRfc3161`/`OpcionesTsa` ya resuelto sin colisión, login end-to-end sigue funcionando. Una firma DNIe/PKCS11 real que efectivamente dispare el flujo completo de `FirmarLocalHandler` (incluido este paso) no fue posible ejercitarla en esta sesión — mismo límite de RUNBOOK 12.9/12.24.

**Alcance no cubierto, documentado explícitamente**:
- Prueba de integración de `ValidadorDocumentoPades.ValidarAsync` que confirme que separa `SellosDeArchivo` de `Firmas` con dependencias reales (`ValidadorCertificados` completo) — la lógica en sí es un `.Where()` de una línea; se evaluó y se decidió no montar el harness completo (raíces, TSL, HTTP) solo para eso, mismo criterio que la prueba de integración de `FirmarLocalHandler` pospuesta en 12.24.
- Renovación periódica del sello de archivo (repetir el `/DocTimeStamp` antes de que el algoritmo/certificado de la TSA usada quede débil) — el mecanismo (repetible, cada llamada es independiente) queda listo; el disparador temporal es un procedimiento operativo, no código.
- Verificación de que la propia TSA sea una autoridad acreditada (cadena de confianza del certificado de la TSA) — misma limitación ya documentada para PAdES-T en RUNBOOK 12.14, hereda igual aquí.

### 12.26 Pruebas PKCS#11 automatizadas contra un SoftHSM2 real (informe, sección 13 — el bloqueo de toolchain de la sección 5 de la matriz de cumplimiento, resuelto)

`ProveedorCriptograficoPkcs11` (RUNBOOK.md sección 10) estaba probado end-to-end contra un DNIe físico real, pero sin ninguna prueba AUTOMATIZADA — el informe de preauditoría exige explícitamente una batería contra PKCS#11 (certificado revocado, expirado, PIN incorrecto, tarjeta retirada). La vía bloqueaba en la falta de un toolchain de compilación C++ (Visual Studio + CMake + vcpkg) para compilar SoftHSM2 en Windows, documentado como pospuesto en `docs/08-cumplimiento/matriz-cumplimiento-indecopi.md` sección 5. Esta sesión instaló el toolchain y cerró el bloqueo.

**Toolchain instalado**: workload "Desarrollo para el escritorio con C++" agregado a la instalación existente de Visual Studio 2022 Community (`vs_installer.exe modify`), CMake 4.4.3 (vía `winget`), vcpkg clonado y bootstrapeado en `C:\vcpkg`. **Detalle real que costó varios intentos**: `vs_installer.exe --quiet` exige ejecutarse elevado — sin eso falla con el código 5007 sin más explicación en la consola (el mensaje real, "Commands with --quiet or --passive should be run elevated from the beginning", solo aparece en el log de `%TEMP%\dd_installer_*.log`, nunca en stdout/stderr) — y, ya elevado, `--installPath` con espacios se corta en el primer espacio si se pasa como `-ArgumentList` array de PowerShell junto con `-Verb RunAs` (ese combo usa `ShellExecute`, que arma la línea de comandos distinto a `-ArgumentList` como array sin `-Verb RunAs`, que usa `CreateProcess`) — el log mostraba literalmente `installPath: C:\Program` truncado. Solución: `-ArgumentList` como un único string con las comillas ya incluidas alrededor de la ruta.

**SoftHSM2 compilado real**: clonado desde `github.com/opendnssec/SoftHSMv2`, dependencias (`openssl`, `cppunit`, `sqlite3` para `x64-windows`) instaladas con vcpkg — con una corrección sobre la guía oficial `CMAKE-WIN-NOTES.md` del propio proyecto, que pide `openssl-windows` (puerto legado, ya no existe en vcpkg moderno; el nombre correcto hoy es simplemente `openssl`). Configurado con CMake (generador `Visual Studio 17 2022`, backend de cripto `openssl`, sin backend de base de datos SQLite — no hace falta para las pruebas), compilado en `RelWithDebInfo` sin errores (solo warnings de conversión `size_t`→`unsigned long`, inofensivos), instalado en `C:\SoftHSMv2\out64`.

**Segundo hallazgo real, de nuevo por el mensaje de error insuficiente**: `softhsm2-util.exe --init-token` fallaba con `LoadLibraryA failed: 0x0000007E` (módulo no encontrado) incluso con todas las DLLs de dependencia presentes (confirmado con `dumpbin /dependents` contra `softhsm2.dll`: `libcrypto-3-x64.dll` y el Universal CRT, todas presentes) — la causa real era que `softhsm2-util` busca el módulo PKCS#11 en una ruta *compilada* por defecto (`DEFAULT_PKCS11_LIB`), no necesariamente la ruta real de instalación configurada vía `CMAKE_INSTALL_PREFIX`. Resuelto pasando `--module <ruta-real>` explícitamente en cada invocación — y confirmado en `ProveedorCriptograficoPkcs11Tests` con la ruta real, no adivinada.

**Token de prueba real**: inicializado con `softhsm2-util --init-token` (PIN `1234`, SO-PIN `5678`), con un certificado autofirmado RSA-2048 generado con `CertificateRequest` de .NET (BasicConstraints CA=false, KeyUsage DigitalSignature+NonRepudiation — misma forma que un certificado de firma real) importado en dos pasos (`--import-type keypair` para la llave PKCS#8 PEM, `--import-type cert` para el certificado X509 PEM, mismo `--id` en ambos para que PKCS#11 los correlacione), bajo la etiqueta `FIR` — la misma que `Pkcs11Options.EtiquetaCertificadoFirma` busca por defecto, así que `ProveedorCriptograficoPkcs11.BuscarCertificadoDeFirma` lo encuentra exactamente como encontraría un DNIe real.

**8 pruebas nuevas** (`tests/SecureSign.UnitTests/Crypto/ProveedorCriptograficoPkcs11Tests.cs`, 117 en total) contra el `ProveedorCriptograficoPkcs11` REAL — no un doble de prueba, la misma clase que habla PKCS#11 de verdad, contra la misma librería nativa que expondría cualquier token real: descubre el certificado de firma por etiqueta; firma y verifica correctamente; una firma sobre contenido alterado no verifica; **PIN incorrecto lanza sin producir ninguna firma** (la fila explícita del informe, sección 13); sin PIN lanza antes de tocar el token; firma en lote con una sola credencial firma las tres operaciones y las tres verifican; un lote con referencias de llave de firmantes distintos lanza (viola la invariante de "mismo firmante, misma tarjeta"); verificar contra un slot inexistente da `false` sin lanzar (nunca colapsa "no se pudo determinar" en una excepción no manejada).

**Por qué estas pruebas solo corren aquí, nunca en CI**: cada una empieza con `if (!ModuloDisponible) return;` — el mismo patrón ya usado en `VerificadorFirmaTslTests` (RUNBOOK 12.22) para un recurso que solo existe en un entorno preparado a mano. CI (Ubuntu, sin este toolchain ni este SoftHSM2 compilado) las salta silenciosamente; se vuelven a activar clonando/compilando SoftHSM2 siguiendo `CMAKE-WIN-NOTES.md` en cualquier máquina Windows con el toolchain — ya no hace falta ningún cambio de diseño de `ProveedorCriptograficoPkcs11` para habilitarlas, tal como ya se documentaba en la matriz de cumplimiento antes de esta sesión.

**Verificado**: 117/117 pruebas en verde (109 previas + 8 nuevas), sin regresiones, build limpio (0 advertencias). `SOFTHSM2_CONF` se dejó como variable de entorno de usuario persistente en esta máquina (`C:\SoftHSMv2\softhsm2.conf`) — sin ella, `C_Initialize` falla con `CKR_GENERAL_ERROR` (el propio módulo SoftHSM2 no encuentra su configuración), un tercer mensaje de error poco informativo que también costó diagnosticar.

**Alcance no cubierto, documentado explícitamente**:
- Certificado revocado/expirado real (el informe también los pide) — no probado en esta ronda: requeriría emitir un certificado con fecha de vencimiento pasada o simular una CRL/OCSP que lo revoque, y esa validación vive en `SecureSign.Trust`/`ValidadorCertificados` (ya probada por separado, RUNBOOK 12.17), no en `ProveedorCriptograficoPkcs11` — el proveedor PKCS#11 en sí no valida vigencia ni revocación, solo firma con la llave que encuentra.
- Retiro de tarjeta a mitad de operación — SoftHSM2 no simula un evento físico de "tarjeta retirada" de la misma forma que un token USB real; el caso más cercano probado (`Verificar_con_slot_inexistente_da_falso_sin_lanzar`) cubre el efecto observable (slot ya no disponible), no el evento en sí.
- Estas pruebas no corren en CI (ver arriba) — la cobertura real de PKCS#11 en cada push sigue siendo cero; la garantía real es que corren en esta máquina, reproducible en cualquier otra siguiendo los mismos pasos.

### 12.27 Secreto interno: rotación sin corte y rechazo de secretos de desarrollo fuera de Development (informe, hallazgo P1 "Secretos productivos")

*(Actualizado en 12.28: la lista `SecretosClienteInternoAdicionales` y el secreto único compartido descritos abajo fueron reemplazados por `Jwt:ServiciosEmisores`, un secreto propio por servicio; la rotación sin corte y la regla de arranque siguen igual, ahora por servicio.)*

`Jwt:SecretoClienteInterno` autentica la llamada de un servicio a `POST /api/auth/interno/emitir` (RUNBOOK 12.21). Seguía siendo un valor fijo de ejemplo dentro de `appsettings.json` de tres servicios, sin procedimiento de rotación, y nada impedía que un despliegue productivo arrancara con él.

**Rotación sin corte** (`JwtOptions.SecretosClienteInternoAdicionales`, solo el Gateway): el endpoint acepta el secreto actual más cualquiera de la lista, comparando contra todos sin cortocircuitar (`AuthController.SecretoInternoValido`) para no filtrar por tiempo cuál coincidió. Procedimiento: (1) poner el secreto nuevo en `Jwt:SecretosClienteInternoAdicionales` del Gateway y desplegarlo; (2) cambiar `Jwt:SecretoClienteInterno` en Signature y Documents al valor nuevo; (3) con todos migrados, mover el nuevo a `Jwt:SecretoClienteInterno` del Gateway y vaciar la lista. Un secreto retirado de la lista deja de valer de inmediato.

**Rechazo al arrancar** (`ValidadorOpcionesJwt`, registrado con `ValidateOnStart` en `AddSecureSignJwtIssuer` y `AddSecureSignTokenExchange`): fuera de `Development`, el servicio no arranca si algún secreto interno empieza con `dev-only-` o tiene menos de 32 caracteres. Los servicios sin secreto configurado (Identity, Evidence, Audit, Crypto) no se ven afectados. En producción el valor se entrega por variable de entorno (`Jwt__SecretoClienteInterno`) o almacén de secretos, nunca en el archivo.

**docker-compose.yml**: gateway, documents y signature ahora fijan `ASPNETCORE_ENVIRONMENT=Development` (es el compose de desarrollo; sin eso el nuevo rechazo tumbaría el stack) y leen `Jwt__SecretoClienteInterno` de `${SECRETO_CLIENTE_INTERNO}` con el valor de ejemplo como respaldo. Un `.env` junto al compose permite cambiarlo sin tocar `appsettings.json`; debe ser el mismo valor en los tres.

**Verificado**: 11 pruebas nuevas (`tests/SecureSign.UnitTests/Auth/SecretoInternoTests.cs`; 128 en total, 0 regresiones, build sin advertencias): aceptación del secreto actual, actual y nuevo durante la rotación, rechazo del retirado, rechazo sin encabezado, un secreto vacío en la lista no autoriza un encabezado vacío, y la regla de arranque (ejemplo permitido en Development, rechazado fuera; secreto corto; secretos adicionales también revisados; secretos largos reales aceptados; servicio sin secreto no falla). Ejecutando el Gateway real con `dotnet run`: en `Production` con el valor de ejemplo termina con `OptionsValidationException` al arrancar; en `Production` con `Jwt__SecretoClienteInterno` de 46 caracteres arranca y escucha; en `Development` arranca con el valor de ejemplo. **Verificado también en Docker** (tras reiniciar Docker Desktop; primer intento caído, mismo problema de RUNBOOK 12.16): gateway, documents y signature arrancan con `ASPNETCORE_ENVIRONMENT=Development`; login OK; `POST /api/auth/interno/emitir` responde 200 con el secreto correcto y 401 con uno incorrecto; con `SECRETO_CLIENTE_INTERNO` de 49 caracteres exportado, el nuevo da 200 y el de ejemplo 401 (la sobreescritura por variable funciona); y el flujo completo 4.2–4.7 (documento, solicitud, visualizar, señal de identidad, firmar, descargar firmado) pasa de punta a punta con ese secreto, o sea la cadena de intercambio interno Firma → Documentos/Identidad/Crypto/Evidencia sigue intacta.

**Alcance no cubierto**: sigue siendo UN secreto compartido entre servicios emisores (uno por servicio queda como mejora futura, RUNBOOK 12.21); la rotación es un procedimiento manual, sin disparador automático; el valor de producción lo entrega el operador (no hay almacén de secretos integrado); rotación de `client_secret` de integradores sigue por `SecretosHash` como lista (RUNBOOK 12.23), sin endpoint de gestión.


### 12.28 Secreto por servicio y política de emisión del Gateway (informe, hallazgo P1 "Secretos productivos"; cierra el "fuera de alcance" de 12.21)

**Hueco que 12.21 dejaba abierto**: `POST /api/auth/interno/emitir` firmaba CUALQUIER conjunto de claims que pidiera quien tuviera el secreto interno, y ese secreto era uno solo compartido por Signature y Documents. Un servicio comprometido podía pedir, por ejemplo, un token de audiencia externa (`securesign-platform`) con todos los scopes de negocio para cualquier tenant, o hacerse pasar por el otro servicio (`act` ajeno) — o sea, comprometer UN servicio equivalía a comprometer la autorización de toda la plataforma, justo lo que la migración a RS256 quería evitar.

**Qué cambió**:
- **Un secreto por servicio.** `Jwt:ServiciosEmisores` (solo Gateway) lista cada servicio autorizado con su `Nombre`, sus `Secretos` (más de uno solo durante una rotación) y `PuedeEmitirTickets`. Cada servicio emisor declara `Jwt:NombreServicio` y su propio `Jwt:SecretoClienteInterno`; `EmisorTokenInterno` los envía en `X-Internal-Service` y `X-Internal-Client-Secret`. El secreto de Documents no autentica a Signature. Un servicio desconocido y un secreto malo dan la misma respuesta (`401 SECRETO_INVALIDO`) y recorren la misma comparación en tiempo constante.
- **Política de emisión** (`PoliticaEmisionInterna`, `403 POLITICA_DE_EMISION` con el motivo). Solo hay dos tipos de token legítimos: (1) **intercambio interno** — audiencia interna, scope exactamente `internal-service`, `act` igual al servicio autenticado, vida ≤ `MinutosExpiracionInterno`, claims solo de `{tenant_id, usuario_id, scope, act}`; (2) **ticket del Firmador Local** — solo servicios con `PuedeEmitirTickets` (Signature), audiencia externa, scope exactamente `firmar:local`, vida ≤ 5 minutos, claims solo de la lista del ticket. Un claim fuera de lista se rechaza, no se ignora.
- **La política no diverge de los emisores reales**: dos pruebas construyen las peticiones con `TokenExchangeService` (`Exchange` y `EmitirTokenDeSistema`) y `EmisorTicketFirmaLocal` reales, capturan lo que mandarían al Gateway y comprueban que la política las acepta. Si alguien agrega un claim a un emisor y olvida la política, esa prueba falla.
- **Rotación sin corte por servicio**: agregar el secreto nuevo a `Secretos` de ese servicio en el Gateway, cambiar `Jwt:SecretoClienteInterno` en el servicio, retirar el viejo. Reemplaza a `SecretosClienteInternoAdicionales` de 12.27. La regla de arranque de 12.27 (rechazo de `dev-only-` y de menos de 32 caracteres fuera de Development) ahora también revisa los secretos de cada servicio del Gateway.
- **docker-compose.yml**: secretos de ejemplo distintos por servicio; sobreescritura con `SECRETO_INTERNO_SIGNATURE` / `SECRETO_INTERNO_DOCUMENTS` en un `.env` (el Gateway y el servicio ven el mismo valor vía `Jwt__ServiciosEmisores__N__Secretos__0` y `Jwt__SecretoClienteInterno`).

**Verificado**: 27 pruebas nuevas en `SecretoInternoTests` (144 en total, 0 regresiones, build sin advertencias): autenticación por servicio (secreto ajeno, servicio desconocido, sin encabezados, rotación, secreto retirado, secreto vacío), cada regla de la política en ambos tipos de token (audiencia, scope, claim extra, `act` ajeno, vida máxima, sin permiso de tickets) y la regla de arranque. En vivo contra el stack Docker real (imágenes reconstruidas): Documents pidiendo un token externo con scopes de negocio recibe `403 POLITICA_DE_EMISION`; Documents usando su secreto para hacerse pasar por Signature recibe `401`; el intercambio legítimo de Documents da `200`; el flujo completo documento → solicitud → visualizar → señal de identidad → firmar → descargar firmado pasa de punta a punta (cadena de intercambio interno intacta), y el ticket del Firmador Local (`ticket-firmador-local`) se emite correctamente.

**Alcance no cubierto**: los secretos siguen en la configuración de cada servicio (variable de entorno o archivo), sin almacén de secretos integrado — el operador los entrega; no hay endpoint de gestión ni disparador automático de rotación; el Gateway no limita la tasa de peticiones a `interno/emitir`; los tokens de intercambio conservan `tenant_id`/`usuario_id` tal como los declara el servicio emisor — la política acota la FORMA del token, no verifica que el servicio los haya tomado de un token de entrada válido (el servicio sigue siendo de confianza para eso).


### 12.29 Limitación de tasa en los endpoints de credenciales del Gateway (informe, hallazgo P1 "Autenticación productiva")

`POST /api/auth/token` verifica un hash Argon2id por intento (RUNBOOK 12.23), deliberadamente caro (~19 MiB de memoria por verificación). Sin tope, ese endpoint público era a la vez un vector de adivinación de `client_secret` y de agotamiento de memoria/CPU del Gateway con peticiones baratas del lado del atacante. `POST /api/auth/interno/emitir` tampoco tenía tope.

**Qué cambió** (`LimitacionDeTasa`, middleware `RateLimiter` de ASP.NET Core): ventana fija de 1 minuto por dirección remota, respuesta `429` con `Retry-After`. Dos políticas independientes, configurables en la sección `LimitacionDeTasa` de `appsettings.json`: `TokenPorMinuto` (10 — un integrador legítimo pide un token por hora) e `InternoPorMinuto` (3000 — lo llaman los servicios en cada intercambio, así que el tope solo frena un abuso descontrolado, no el uso normal, incluido el modo masivo). Agotar una política no afecta a la otra.

**Verificado**: 5 pruebas nuevas (`LimitacionDeTasaTests`, 149 en total, 0 regresiones; `Microsoft.AspNetCore.TestHost` agregado al proyecto de pruebas) contra un servidor HTTP en memoria con el middleware y la configuración reales: responde 429 con `Retry-After` entre 1 y 60 al pasar el límite, las dos políticas son independientes, el tope interno funciona, los valores por defecto son los documentados, y un test de reflexión confirma que `ObtenerToken` y `EmitirInterno` declaran su política (si alguien quita el atributo, falla). En vivo contra el stack Docker (Gateway reconstruido): 12 intentos seguidos con un secreto incorrecto dan 10 × `401` y luego 2 × `429` con `Retry-After: 60`, y `interno/emitir` sigue respondiendo `200` en ese mismo momento.

**Alcance no cubierto**: la partición es por IP del par TCP — detrás de un balanceador o proxy inverso propio hay que habilitar `ForwardedHeaders` para que sea la IP del cliente real (si no, todos comparten el cupo del proxy); no hay bloqueo por `client_id` (un atacante distribuido en muchas IP no queda frenado por este mecanismo — eso lo cubriría un WAF o el IdP externo); el estado de los contadores vive en memoria de cada instancia del Gateway (con varias réplicas, el tope efectivo es N veces mayor); las rutas proxied hacia los demás servicios no tienen limitación de tasa propia.


### 12.30 Secretos entregados como archivos: Docker/Kubernetes secrets (informe, hallazgo P1 "Secretos productivos")

Hasta 12.29 los secretos internos solo podían venir de `appsettings.json` (con el valor `dev-only-...` de ejemplo) o de variables de entorno — visibles con `docker inspect` y `/proc/<pid>/environ`. El formato estándar de los orquestadores para secretos es un ARCHIVO por secreto (`/run/secrets` en Docker/Swarm, volumen de Secret en Kubernetes).

**Qué cambió** (`SecretosDeArchivo`, `AddSecureSignSecretosDeArchivo`, llamado al inicio de `Program.cs` de Gateway, Documents y Signature): al arrancar lee cada archivo de `/run/secrets` (o de `SECURESIGN_SECRETS_DIR`) y lo agrega como configuración — el nombre del archivo es la clave con `__` en lugar de `:` (`Jwt__SecretoClienteInterno`, `Jwt__ServiciosEmisores__0__Secretos__0`), el contenido es el valor sin el salto de línea final. Se agrega DESPUÉS de appsettings y de las variables de entorno, así que tiene la precedencia más alta; un directorio inexistente se ignora (desarrollo sigue igual). Ejemplo de declaración con Compose en el encabezado de `docker-compose.yml`. Los archivos que empiezan con `.` se saltan (metadatos de los volúmenes de Kubernetes).

**Decisión con evidencia**: primero se usó `AddKeyPerFile` de Microsoft; la prueba del salto de línea final falló — `AddKeyPerFile` NO lo recorta, así que `echo secreto > archivo` dejaba `"secreto\n"` como valor, distinto del secreto que envía el otro extremo (autenticación rota en silencio, y con longitud mínima falseada en +1). Se reemplazó por un lector propio de 15 líneas que recorta `\r\n` finales. Se lee una sola vez al arrancar: rotar un secreto = reiniciar el servicio.

**Mejora colateral**: el mensaje de la regla de arranque (12.27) ahora nombra el origen exacto del secreto rechazado (`Jwt:SecretoClienteInterno` o `Jwt:ServiciosEmisores['<servicio>']`) y sugiere el archivo de secretos; antes repetía el mismo texto genérico sin decir cuál. Se eliminó `JwtOptions.SecretosInternosAceptados()`, que quedó sin usos desde 12.28.

**Verificado**: 6 pruebas nuevas (`SecretosDeArchivoTests`, 155 en total, 0 regresiones, build sin advertencias): el nombre con `__` se convierte en clave (incluidos los índices de `ServiciosEmisores`), el salto de línea final (`\n` y `\r\n`) no forma parte del secreto, el archivo gana sobre appsettings sin borrar lo que no está en archivo, directorio inexistente no lanza, un secreto en archivo satisface la regla de arranque fuera de Development, y la variable `SECURESIGN_SECRETS_DIR` redefine el directorio. **En vivo con la imagen Docker real del Gateway, en `ASPNETCORE_ENVIRONMENT=Production`** (el caso que hasta ahora solo se probaba con `dotnet run`): sin secretos falla al arrancar con `OptionsValidationException`; con dos archivos montados en `/run/secrets:ro` arranca y escucha; `interno/emitir` acepta el secreto del archivo (`200`) y rechaza el `dev-only-...` que sigue en el appsettings de la imagen (`401`).

**Alcance no cubierto**: no es un vault — no hay rotación automática ni auditoría de acceso al secreto (eso lo darían Azure Key Vault/HashiCorp Vault/AWS Secrets Manager; este mecanismo es compatible con ellos vía sus agentes de inyección de archivos); se lee una vez al arrancar, sin recarga en caliente; Documents y Signature verifican el mismo mecanismo pero solo el Gateway se probó en Production dentro de un contenedor.


### 12.31 Endurecimiento HTTP del Gateway: cabeceras de seguridad y CORS cerrado por defecto (preparación del pentest externo)

Revisión propia del borde público antes del pentest de terceros. Dos hallazgos reales:

1. **`Cache-Control: no-store` ausente en la respuesta del endpoint de token.** RFC 6749 §5.1 lo EXIGE (`Cache-Control: no-store` y `Pragma: no-cache`) porque la respuesta contiene un `access_token`; el Gateway no lo enviaba, y tampoco en las respuestas proxied de documentos y evidencias, que no deben quedar en cachés compartidas. Tampoco había ninguna cabecera de seguridad (`nosniff`, anti-framing, `Referrer-Policy`, CSP).
2. **CORS `AllowAnyOrigin` permanente.** La política del visor de firma estático se aplicaba SIEMPRE, también en producción (el comentario decía "en producción esto se restringe", pero no había ningún código que lo hiciera): cualquier página web podía leer respuestas del Gateway con un token robado o filtrado.

**Qué cambió** (`SeguridadHttp`):
- `UseCabecerasDeSeguridad`, en TODA respuesta del Gateway (propia o proxied): `Cache-Control: no-store` + `Pragma: no-cache` cuando la respuesta no trae su propio `Cache-Control` (uno propio no se pisa), `X-Content-Type-Options: nosniff` (un PDF nunca se reinterpreta como HTML/script), `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'` (el Gateway sirve JSON y binarios, jamás una página; se omite solo en `/swagger`, que existe solo en Development). `UseHsts` fuera de Development.
- `AddCorsGateway`: en **Development** sigue abierto (el visor `firma-web` se abre desde `file://`); fuera de Development solo los orígenes de `Cors:OrigenesPermitidos` (lista en `appsettings.json`, vacía por defecto) y, **sin ninguno configurado, no se emite ninguna cabecera CORS** — cerrado por defecto. Las cabeceras expuestas de documento (`X-Hash-Documento`, `X-Evidencia-Url`, `Content-Disposition`) se conservan.

**Verificado**: 8 pruebas nuevas (`SeguridadHttpTests`, 163 en total, 0 regresiones, build sin advertencias) contra un servidor HTTP en memoria con el código real: todas las cabeceras presentes, `no-store` por defecto, un `Cache-Control` propio no se pisa, Swagger sin CSP pero con el resto, Development acepta cualquier origen, Production sin orígenes no emite CORS, Production con un origen configurado solo lo acepta a él (uno ajeno no recibe `Access-Control-Allow-Origin`) y conserva las cabeceras expuestas. **En vivo con el Gateway real** (`dotnet run`, `ASPNETCORE_ENVIRONMENT=Production`, secretos por variable de entorno): `POST /api/auth/token` con `Origin: https://sitio-malicioso.evil` responde `200` con `Cache-Control: no-store`, `Pragma: no-cache`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` y CSP, y **sin** ninguna cabecera `Access-Control-*`. **Además verificado en el stack Docker** (con 12.32 ya aplicado): el mismo `POST /api/auth/token` con `Origin` hostil responde con `Cache-Control: no-store`, `Pragma: no-cache`, `X-Frame-Options: DENY` y CSP (y `Access-Control-Allow-Origin: *` porque el compose corre en Development, como se diseñó), y la descarga del PDF firmado responde con `Cache-Control: no-store`, `X-Content-Type-Options: nosniff` y `Content-Type: application/pdf`.

**Alcance no cubierto**: las cabeceras se fijan en el Gateway, no en los servicios internos (no son alcanzables desde fuera); no hay `Permissions-Policy` ni `Cross-Origin-*-Policy` (poco valor para una API JSON); HSTS lo emite `UseHsts` solo sobre HTTPS — la terminación TLS y `ForwardedHeaders` son del despliegue; el Firmador Local (ejecutable WinForms local) tiene su propio CORS, no cubierto aquí (RUNBOOK 12.13). Un pentest de terceros sigue pendiente — esta ronda solo cierra lo que una revisión propia encontró.


### 12.32 El endpoint que acuña tokens deja de ser alcanzable desde el puerto público del Gateway (preparación del pentest externo)

**Hallazgo de la revisión propia**: `POST /api/auth/interno/emitir` es un endpoint del PROPIO Gateway (no una ruta proxied), o sea que vivía en el mismo listener que la API pública: cualquiera con acceso a `https://api...` podía golpear el endpoint que firma tokens. Estaba protegido (secreto propio por servicio, política de emisión, limitación de tasa: 12.27–12.29) pero es exactamente el endpoint que un pentest señala primero, y no tiene ninguna razón de ser público.

**Qué cambió**: el Gateway escucha en DOS puertos y `interno/emitir` solo responde en el interno.
- `Jwt:PuertoInterno` (Gateway): puerto local de Kestrel en el que únicamente se sirve el endpoint. En cualquier otro puerto responde `404` — incluso con el secreto correcto y con uno incorrecto, así que desde fuera no se distingue "no existe" de "secreto malo". Se compara contra `HttpContext.Connection.LocalPort` (el puerto real de la conexión), no contra el header `Host`, que el cliente controla — un `Host: gateway:8081` puesto a mano en el puerto público no lo abre.
- `Jwt:AuthorityInterna` (Signature, Documents): URL del listener interno (`http://gateway:8081`) a la que `EmisorTokenInterno` pide los tokens; si no se define, usa `Authority`. La validación de tokens (JWKS) sigue usando `Authority`.
- **Regla de arranque** (extiende 12.27): fuera de Development, un Gateway con `ServiciosEmisores` y sin `PuertoInterno` no arranca — seguro por defecto, no por documentación.
- **docker-compose.yml**: gateway con `ASPNETCORE_HTTP_PORTS: "8080;8081"` y `Jwt__PuertoInterno: "8081"`; solo `8080` se publica al host; Signature y Documents con `Jwt__AuthorityInterna: "http://gateway:8081"`. En producción, publicar solo el puerto público en el balanceador y dejar el interno dentro de la red del clúster.

**Verificado**: 6 pruebas nuevas (169 en total, 0 regresiones, build sin advertencias): 404 en el puerto público (con secreto correcto y con incorrecto), funcionamiento en el interno, sin `PuertoInterno` no se restringe (Development/compatibilidad), la regla de arranque lo exige fuera de Development y lo deja opcional en Development. **En vivo contra el stack Docker real** (Gateway, Documents y Signature reconstruidos): `POST localhost:8080/api/auth/interno/emitir` con el secreto CORRECTO de Documents responde `404`; `docker compose ps` confirma que solo `8080` está publicado; y el flujo completo documento → solicitud → visualizar → señal de identidad → firmar → descargar firmado pasa de punta a punta, lo que solo es posible si Signature y Documents efectivamente obtienen sus tokens por el listener `8081`. (Nota de la verificación: `localhost:8081` en esta máquina Windows responde `404` porque el puerto lo ocupa el proceso del sistema `PID 4`, no el contenedor — `docker compose ps` es la evidencia de que el contenedor no lo publica.)

**Alcance no cubierto**: la separación es por puerto, no por autenticación mutua (mTLS) entre servicios — quien alcance la red interna con un secreto válido sigue pudiendo pedir tokens dentro de la política; la segmentación de red (que el puerto interno no se publique) es responsabilidad del despliegue, el código solo garantiza que el endpoint no responde en el puerto público; el resto de endpoints de servicios internos (`/api/interno/identidad/*`, atajo deliberado documentado en el README) siguen accesibles por el Gateway como antes.


### 12.33 Prueba de punta a punta del flujo de firma y el hallazgo de que PAdES-LT no se usaba al validar (informe, sección 13 "Pruebas que faltan")

**Qué se construyó**: `FirmaLocalDePuntaAPuntaTests` (`tests/SecureSign.UnitTests/Integracion`), 17 pruebas que ejecutan `FirmarLocalHandler` → `ValidadorConfianzaFirmanteIofe` → `ValidadorCertificados` (cadena X.509, TSL, CRL, OCSP) → PAdES-LT (`PdfDssWriter`) → PAdES-LTA (`ClienteTsaRfc3161` + sello de archivo) → `ValidadorDocumentoPades` sobre lo que el handler dejó guardado, con las piezas REALES. Solo son dobles los bordes: HTTP (una CA de prueba que sirve AIA, CRL y OCSP firmados con llaves reales, y una TSA de prueba con el protocolo RFC 3161 real), el repositorio y los clientes hacia otros servicios. La llave que firma es RSA en memoria, no un token: el camino PKCS#11 ya se prueba contra un SoftHSM2 real (12.26) y el Firmador Local (WinForms) no es referenciable desde las pruebas — así que esto cubre la cadena de confianza y de evidencia, no el acto de firmar con un token.

**Casos cubiertos**: camino feliz (PAdES con `/DSS` y `/DocTimeStamp`, el validador independiente lo da por válido y el sello de archivo por válido); certificado **revocado** (por CRL y OCSP), **expirado**, de una CA **fuera de la TSL**, **CA sin red** (fail closed: "no pude comprobarlo" nunca es "no está revocado"), índice de confianza insuficiente, PDF alterado tras firmar, PAdES firmado con un certificado distinto al de la firma desacoplada, ticket con hash que ya no coincide — todos rechazan SIN efectos (no se guarda documento, no se marca firmado, no se registra evidencia de firma) y los de confianza/ticket quedan en auditoría, y cada uno de los cuatro primeros verifica el MOTIVO específico en el mensaje, no solo que falló; si la TSA no responde la firma igual se completa como PAdES-LT sin sello de archivo (best-effort, como se diseñó); una alteración posterior al guardado la detecta el validador.

**Hallazgo real que salió de escribir la prueba**: PAdES-LT (12.24) **escribía** el DSS pero nada lo **leía**. `ValidadorDocumentoPades` siempre consultaba la CA en vivo, así que el material embebido no servía para el único propósito de LT: validar el documento cuando la CRL/OCSP originales ya no están publicados. Un documento firmado hoy quedaba imposible de validar el día que la CA dejara de responder — indistinguible de PAdES-B/T. Lo escribimos "completo a nivel de código" en 12.24/12.25 sin ejercitar esa mitad.

**Qué cambió**:
- `PdfDssReader` (Pades): lee el `/DSS` más reciente (certificados, CRLs, OCSPs) cortando por `/Length` (el DER podría contener por casualidad "endobj"); ante un PDF que no puede recorrer devuelve `null`, nunca lanza.
- `VerificadorRevocacionCrl.VerificarEmbebida` / `VerificadorRevocacionOcsp.VerificarEmbebida` (Trust): la misma verificación de firma que en la descarga (CRL/OCSP contra el emisor del certificado) pero sobre material embebido y **evaluando la vigencia en el instante de la firma, no "ahora"** — una CRL embebida hace años ya venció, pero era vigente cuando se firmó.
- `ValidadorCertificados.ValidarAsync(..., embebido)`: con material embebido valida SIN red (intermedios del DSS en vez de AIA; CRL/OCSP embebidos). La cadena y la TSL siguen exigiéndose completas contra las raíces configuradas: el DSS no es confiable por sí mismo, solo permite validar sin red.
- `ValidadorDocumentoPades`: con DSS, valida primero con el material embebido; si no alcanza para concluir "confiable" cae a la red (comportamiento anterior) y deja el motivo en el expediente; **si el material embebido dice "revocado", no se le da otra oportunidad a la red**.

**Verificado**: 22 pruebas nuevas (191 en total, 0 regresiones, build sin advertencias): las 17 de punta a punta y 5 de `PdfDssReaderTests` (lo escrito es exactamente lo leído, sin DSS es null, dos firmantes, bytes con "endobj" dentro, basura). Las que prueban LT: con la CA caída el documento guardado sigue siendo válido usando el DSS (y la evidencia lo dice); el MISMO documento sin DSS y con la CA caída NO se puede validar (control); un DSS armado a mano con una CRL genuina valida sin red (control del montaje) mientras uno con una CRL firmada por otra llave NO; y un DSS cuya CRL lista al certificado como revocado gana aunque la red diga "no revocado". Humo en Docker: `Signature.Api` reconstruido arranca limpio y `POST /api/validador/pdf` sigue respondiendo con el mismo expediente para un PDF sin firmas. El camino con DSS real no se ejerció en Docker (el flujo Docker no produce PAdES; solo el Firmador Local lo hace).

**Alcance no cubierto**: (1) el acto de firmar con un token real (DNIe): esta prueba usa una llave en memoria; (2) simplificación en el tiempo de la revocación — no se exige la regla estricta de ETSI EN 319 102-1 de que la CRL/OCSP sea POSTERIOR a la firma, solo que no hubiera vencido antes de ella; (3) el DSS se lee completo sin filtrar por la entrada `/VRI` de cada firma (seguro porque cada pieza se verifica criptográficamente, pero un DSS con material de más no se marca como anómalo); (4) la cadena de confianza de la TSA del sello de archivo sigue sin verificarse (12.14/12.25); (5) el sello de archivo (LTA) no interviene en el veredicto de validez, igual que antes.


### 12.34 EKU y CertificatePolicies: siempre se leen y se reportan, exigirlos es una decisión de configuración (informe, sección 8 "Certificado: no basta encontrar 'FIR'")

**Situación**: el motor de confianza verificaba BasicConstraints y KeyUsage (calibrado con un DNIe real, 12.9) pero ignoraba por completo ExtendedKeyUsage y CertificatePolicies. Exigirlos a ciegas produciría falsos rechazos: no existe todavía una referencia confiable del OID de política IOFE y el DNIe real declara un EKU ("Secure Email") que no es específico de firma de documentos. Pero ignorarlos también dejaba al auditor sin ver qué política declara cada certificado, que es precisamente lo que el informe pide poder revisar.

**Qué cambió**:
- `AnalizadorPoliticaCertificado` (Trust): SIEMPRE lee los OID de ExtendedKeyUsage y de CertificatePolicies (2.5.29.32, RFC 5280 §4.2.1.4: solo los `policyIdentifier`, no los calificadores) y los deja en el expediente de validación (líneas de evidencia, y `ResultadoValidacionCertificado.Politica`). Una extensión mal formada se lee como vacía, nunca tumba la validación.
- `OpcionesPoliticaCertificado` (sección `PoliticaCertificado` de appsettings de Signature.Api): `OidsPoliticaPermitidos`, `OidsEkuPermitidos` y `Exigir`. **Por defecto todo vacío y `Exigir=false`: no se rechaza nada** (estado `SoloInformativa`). Con listas configuradas el estado es `Cumple`/`NoCumple` (deben cumplirse AMBAS listas si las dos están configuradas; una lista vacía no se evalúa); con `Exigir=true` un certificado que no cumple deja de tener propósito válido — no es confiable, se rechaza el `FirmarLocal` y queda en auditoría (`CertificadoRechazadoPorConfianza`) y el validador independiente lo marca inválido; con `Exigir=false` el incumplimiento solo queda escrito en la evidencia. Cuando INDECOPI/RENIEC publiquen la lista oficial de OID de política, activar la exigencia es cambiar la configuración, no el código.
- La API del validador (`POST /api/validador/pdf`) devuelve por firma `extendedKeyUsages`, `politicasCertificado` y `estadoPolitica`.

**Verificado**: 15 pruebas nuevas (206 en total, 0 regresiones, build sin advertencias): 9 unitarias del analizador (lee EKU y políticas, sin extensiones = vacío, extensión mal formada no lanza, coincidencia/no coincidencia, EKU y política deben cumplirse ambos, listas vacías = no evaluar, `Exigir` sin listas no rechaza) y 6 de punta a punta con el `FirmarLocalHandler` real (por defecto se reporta y no rechaza, incluido un certificado con el EKU "Secure Email" del DNIe; sin extensiones se reporta "no declarado"; exigida y cumple → acepta; exigida y no cumple → rechaza con el motivo `EXIGIDA` y auditoría; exigida y el certificado no declara ninguna política → rechaza; configurada pero no exigida → acepta y registra `NoCumple`). Una de las 9 unitarias lee los DOS certificados REALES del repositorio (raíz ECERNEP y firmante de la TSL de INDECOPI) y compara los OID extraídos contra un oráculo independiente (BouncyCastle): la raíz real declara `anyPolicy` (2.5.29.32.0), que .NET formatea como texto localizado, por eso el oráculo no es `X509Extension.Format`. `Signature.Api` reconstruido en Docker arranca limpio con la política registrada por DI y `POST /api/validador/pdf` sigue respondiendo igual.

**Alcance no cubierto**: no se verificó contra un certificado de firma REAL de un ciudadano (DNIe) — solo contra certificados de CA reales y certificados de prueba; no hay lista oficial de OID a configurar todavía, así que la exigencia queda apagada; solo se compara el certificado hoja (no se exige que las políticas de la cadena sean consistentes ni se procesan `policyMappings`/`policyConstraints`, RFC 5280 §6.1); el visor `validador-web` todavía no muestra los tres campos nuevos.


### 12.35 Instalador MSI del Firmador Local y pipeline de firma Authenticode (informe, hallazgo P0-06 y §14 "release controlado")

**Punto de partida**: el Firmador Local se distribuía como un `.exe` suelto (con .NET instalado aparte) y la matriz decía "no existe instalador todavía". P0-06 exige autenticidad e integridad del software distribuido. Lo único que NO es ingeniería es el certificado de firma de código (procura); todo lo demás se construyó para que, el día que llegue el certificado, solo haya que cargarlo como secreto.

**Qué se construyó** (`src/backend/src/Tools/SecureSign.FirmadorLocal/installer/`):
- `FirmadorLocal.wxs` — MSI de WiX 5.0.2 (MS-RL), **por usuario** (no pide administrador: el Firmador solo usa HKCU y escucha en 127.0.0.1). Instala en `%LOCALAPPDATA%\Programs\SecureSign Firmador Local` el runtime **autocontenido** (170 MB publicado, MSI de 58,7 MB: el usuario no tiene que instalar .NET), registra el protocolo `securesign://` con los mismos valores que `--registrar`, el inicio automático en la bandeja (**por defecto activado, desactivable con `INICIAR_CON_WINDOWS=0`** — decisión de producto que hay que confirmar: sin ella el usuario debe abrir el programa a mano en cada sesión) y un acceso directo en el menú Inicio. La desinstalación limpia todo. `MajorUpgrade`: una versión nueva reemplaza a la anterior y una anterior no pisa a una más reciente.
- `Firmar-Binarios.ps1` — firma con `signtool` (SHA-256 + sello de tiempo RFC 3161, 3 reintentos ante fallo del servidor de sellos) todos los `.exe`/`.dll`/`.msi` que aún no tengan firma, y **verifica cada uno después**: firma presente, huella del firmante igual a la del PFX usado y sello de tiempo presente. Falla si falta el certificado, si no tiene el uso "Firma de código" (1.3.6.1.5.5.7.3.3), si venció, o si algo no verifica; `-PermitirNoConfiable` existe solo para probar con certificados autofirmados (acepta cadena no confiable, nunca una firma que no corresponda al contenido).
- `Construir-Instalador.ps1` — un solo script para CI y máquina local: publicar → firmar binarios → empaquetar MSI → firmar MSI → manifiesto SHA-256 que declara explícitamente si el artefacto está **FIRMADO** o **SIN FIRMA — no distribuir a usuarios finales**. Los binarios se firman ANTES de empaquetar para que lo que queda instalado también lleve firma, no solo el contenedor.
- `Probar-Instalador.ps1` — prueba real: instala, comprueba archivos, versión, registro, inicio automático y acceso directo, desinstala y comprueba que no queda nada; repite con `INICIAR_CON_WINDOWS=0`. Respalda y restaura el estado previo del usuario y aborta si ya existe la carpeta de instalación.
- `.github/workflows/ci.yml` (job `release-manifest-firmador`, solo push a `main`): instala WiX, decodifica el secreto `CODESIGN_PFX_BASE64` a un archivo temporal si existe (lo borra al terminar, también si falla), construye, **prueba el instalador en el runner limpio** y publica MSI + manifiesto. **Sin el secreto genera el MSI sin firma y emite una anotación de advertencia** — nunca firma en silencio con otra cosa. Secretos a cargar cuando exista el certificado: `CODESIGN_PFX_BASE64` y `CODESIGN_PFX_PASSWORD`.

**Verificado localmente** (Windows 11, sin elevación): con un certificado de PRUEBA autofirmado (contraseña sintética, exportado a un PFX temporal y retirado del almacén del usuario) se construyó de punta a punta un MSI FIRMADO: 9 binarios + el MSI firmados con **sello de tiempo real de DigiCert (RFC 3161)**, verificados por el script; `Get-AuthenticodeSignature` da `UnknownError` (esperado: raíz autofirmada no confiable) con el firmante y el sello correctos; **un byte alterado en el `.exe` firmado da `HashMismatch`**; sin certificado y con un certificado sin el uso de firma de código el script falla con mensaje claro. `Probar-Instalador.ps1` sobre el MSI firmado: instalación por usuario con código 0, más de 100 archivos, versión 1.0.4, inicio automático, acceso directo, desinstalación limpia (carpeta, inicio automático y acceso directo eliminados) y reinstalación con `INICIAR_CON_WINDOWS=0` sin inicio automático. Además, con un nombre de clave de protocolo distinto (`securesignprueba`) el MSI escribe y elimina el protocolo correctamente con raíz HKCU y con HKCR.

**Hallazgos y decisiones con evidencia**:
- **WiX**: la serie 4.0.6 no tiene el elemento `Files` (llega en 5.x), así que se usa **5.0.2**. Las series 6+ están bajo una licencia distinta (cuota de mantenimiento de código abierto, según su publicación; no verificado aquí) — no se usó; se instaló 7.0.0 por error y se desinstaló sin aceptar ningún EULA. Herramienta instalada con permiso explícito del usuario.
- **Un antivirus bloqueó el script de firma** (acceso denegado y desaparición del archivo durante varios minutos) y se aisló la causa por bisección: no fue el nombre ni `signtool`, fue la COMBINACIÓN "decodificar un secreto base64 a un archivo temporal + invocar una herramienta con la contraseña". Rediseño: `Firmar-Binarios.ps1` **nunca decodifica secretos** (recibe la ruta de un PFX); el paso de CI que precede es quien lo decodifica. Mejor diseño de todos modos (menos superficie en el script). **Quedan dos archivos bloqueados sin poder borrarse (`installer/Firmar-Authenticode.ps1` y `installer/Firmar-Codigo.ps1`): son restos sin uso y NO deben commitearse; bórralos a mano.**
- **Protocolo `securesign://` en ESTA máquina no se pudo verificar**: con la clave `securesign` ya registrada (instalación de desarrollo) el MSI no la sobrescribió, y tras borrarla el MSI tampoco la creó — sin explicación, mientras que con un nombre distinto todo funciona (ver arriba). Por eso `Probar-Instalador.ps1` **salta** esas comprobaciones cuando el usuario ya tiene el protocolo y no lo pisa; se verifican en el runner limpio de CI. Hasta que ese job pase, el registro del protocolo por el MSI debe considerarse **sin verificar**. (El protocolo es el modo heredado; el modo principal es el servicio HTTP local en 127.0.0.1, que no depende de él.)
- Los scripts `.ps1` se guardan con BOM UTF-8: Windows PowerShell 5.1 los lee como ANSI sin él y rompe los acentos.

**Alcance no cubierto**: no hay certificado real, así que **el MSI que produce CI hoy no está firmado** (SmartScreen lo marcará; además un certificado nuevo no-EV tarda en ganar reputación); la clave de firma como PFX en un secreto de CI es lo mínimo aceptable — lo recomendable es un HSM/servicio de firma en la nube para que la clave nunca salga; el MSI no tiene interfaz de instalación (instala en silencio con barra de progreso; agregar diálogos exige otra extensión de WiX); no hay auto-actualización (decisión de `politica-versiones-y-cambios.md`); el runtime autocontenido queda congelado en cada versión — un parche de seguridad de .NET exige publicar un MSI nuevo; la versión es `1.0.<número de ejecución de CI>`; se probó por usuario sin elevación, no una instalación por máquina.
