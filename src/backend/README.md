...
|---|---|
| `ProveedorCriptograficoSoftware` (Crypto.Infrastructure) | Llaves ECDSA reales, pero generadas y guardadas en memoria del proceso — **no en un HSM**. La firma es criptográficamente válida, pero no cumple los requisitos de custodia de la Ley 27269 para "firma digital" con presunción legal plena |
...
| Formato de firma embebida (PAdES) | La firma se genera y se puede consultar, pero **no se embebe dentro del PDF** — se sirve como firma detached junto con el documento original |
...
2. Implementar el adaptador PKCS#11 o el SDK de Key Vault en `SecureSign.Crypto.Infrastructure`, respetando la interfaz `IProveedorCriptografico` (ningún consumidor debería cambiar).
