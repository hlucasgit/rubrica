...
| Formato de firma embebida (PAdES) | **Hecho para el Firmador Local** (`RUNBOOK.md` sección 12.8, proyecto `SecureSign.Pades`): el PDF que descarga `GET /api/documentos/{id}/firmado` trae un diccionario `/Sig` real con `/ByteRange` y un CMS/CAdES-BES incrustado, verificable con cualquier lector PAdES estándar — verificado de forma independiente con BouncyCastle en esta sesión. Limitación pendiente: es una reescritura completa por firma, no una actualización incremental ISO 32000 en el sentido estricto, así que en un documento con VARIOS firmantes solo la firma incrustada más reciente queda con un `/ByteRange` válido (ver 12.8). El **sello visual** (nombre, fecha, código de verificación — `RUNBOOK.md` sección 11 e `IEstampadorVisualDocumento`) sigue siendo un artefacto aparte, sin cambios |
