...
| Formato de firma embebida (PAdES) | La firma criptográfica se genera y se puede consultar, pero **no se embebe dentro del PDF** como un diccionario `/Sig` real — se sirve como firma detached junto con el documento original. Sí existe un **sello visual** (nombre, fecha, código de verificación en la posición que el firmante eligió — ver `RUNBOOK.md` sección 11 e `IEstampadorVisualDocumento`), pero es un PDF re-generado aparte con PdfSharpCore, no una actualización incremental PAdES que preserve el byte-range firmado |
